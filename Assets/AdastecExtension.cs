...
void Update()
    ...
    public GameObject Jaywalks;
    public void ActivateSpawnsAndJaywalks(float value)
    {
        PickJaywalk((int)value - 1);
    }
    private void PickJaywalk(int jaywalk)
    {
        for(int i = 0; i < Jaywalks.transform.childCount; i++)
        {
            if(i == jaywalk)
            {
                Jaywalks.transform.GetChild(i).gameObject.SetActive(true);
            }
            else
            {
                Jaywalks.transform.GetChild(i).gameObject.SetActive(false);
            }
        }
    }
}
